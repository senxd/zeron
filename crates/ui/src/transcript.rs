...
use gpui::{
    AnyElement, App, BorderStyle, Bounds, ClipboardItem, Context, Entity, Focusable, ListAlignment,
...
impl Transcript {
    ...
    fn annotation_link_ui(
        ...
        let workspace = self.workspace_link.clone();
        let weak = cx.weak_entity();
        Some(render::LinkUi {
            handler: Rc::new(move |url, window, cx| {
                if let Some(index) = crate::annotations::parse_marker_url(url) {

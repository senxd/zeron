...
pub struct AcpHarness {
    ...
    /// `enabledModels` can differ per workspace. Only the raw advertised list
    /// is cached; patterns are re-applied per call.
...
impl AcpHarness {
    ...
    async fn discover_models(&self, cwd: Option<&Path>) -> Result<Vec<Model>, HarnessError> {
        ...
        let (_scratch, mut child, stderr_tail) =
            self.spawn_agent(Some(&cwd_text), false, &[]).await?;
